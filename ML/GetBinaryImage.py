    ...
    def _image_background(self, img_name, brg_choice, brg_compare, brg_value):
        ...
        for i in range(img.shape[0]):
            for j in range(img.shape[1]):
                if img[i][j][brg_choice] > brg_compare:
                    change_convert_image.putpixel((j, i), brg_value)
        change_convert_image.save(img_name, "PNG")

    def save_binary_to_image(self, path, brg_choice, brg_compare, brg_value):
        path = Preprocessing.append_slash(path)
        img_name = path + 'binary_' + self.image_name + '.png'
        cv2.imwrite(img_name, self.binary_image)
        self._image_background(img_name, brg_choice, brg_compare, brg_value)
